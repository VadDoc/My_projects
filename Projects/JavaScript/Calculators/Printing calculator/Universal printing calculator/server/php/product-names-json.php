...
$filename = 'data-product-names.json'; 
...
if (file_exists($filename)) { 
    ...
    $file = file_get_contents('data-product-names.json'); 
    // Если такого файла нет… 
} else { 
    // …то создаём его сами 
    $file = fopen("data-product-names.json", "a+"); 
} 
...
// file_put_contents('data-product-names.json', json_encode($file)); 
file_put_contents('data-product-names.json', json_encode($file, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)); 
// …и сразу считываем их обратно 
$file = file_get_contents('data-product-names.json'); // Открыть файл data-product-names.json 
