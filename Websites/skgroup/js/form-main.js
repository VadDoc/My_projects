...
function checkForm() {
    ...
    for (let i = 0; i < mailForm.length; i++) { // 
        mailForm[i].addEventListener('change', function() {
            ...
            if(mailForm[7].value == 'mail') {
               fillFormMail(); 
            } else if (mailForm[7].value == 'phone') {
               fillFormPhone();
            }
        })
    }
...
function fillFormPhone() {
    ...
    for (let i = 0; i < mailForm.length - 7; i++) { // 
        mailForm[i].addEventListener('change', function() {
...
function sendDataJSON(data, mailForm, arrPlaceholder, errorMess, serverName) {
    ...
    fetch(`https://sk-group.kiev.ua/wp-content/themes/skgroup/php/${serverName}-form.php`, {
        method: 'POST',
        headers: {
